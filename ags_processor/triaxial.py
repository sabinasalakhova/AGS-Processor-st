"""
Triaxial Test Data Processing Module

Comprehensive triaxial test processing functions from legacy agsfileanalysis.
Generates triaxial summary tables, maps lithology, calculates s-t values,
and handles duplicate removal.
"""
...
def generate_triaxial_table(groups: Dict[str, pd.DataFrame]) -> pd.DataFrame:
    ...
    groups : Dict[str, pd.DataFrame]
        Dictionary of AGS group DataFrames
    
    Returns
    -------
    pd.DataFrame
        Triaxial summary table
    """
...
def generate_triaxial_with_lithology(groups: Dict[str, pd.DataFrame]) -> pd.DataFrame:
    """
    Build triaxial summary table and map lithology from GIU group based on depth ranges.
    ...
    -------
    pd.DataFrame
        Triaxial table with lithology column
    """
...
def calculate_s_t_values(df: pd.DataFrame) -> pd.DataFrame:
    ...
    ----------
    df : pd.DataFrame
        Triaxial data with CELL, DEVF, PWPF columns
    
    Returns
    -------
    pd.DataFrame
        DataFrame with s-t calculations
    """
    df = df.copy()
    ...
    df["t"]           = 0.5 * (df["SIGMA_1"] - df["SIGMA_3"])

    # Choose s: use effective if both SIGMA_1_EFF and SIGMA_3_EFF are finite
...
def remove_duplicate_tests(df: pd.DataFrame) -> pd.DataFrame:
    """
    Remove duplicate triaxial test rows based on key identifiers.
